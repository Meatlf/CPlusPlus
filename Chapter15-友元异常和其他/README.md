...
首先定义Tv类。可以用一组**状态成员**（描述电视机各个方面的变量）来表示电视机。下面是一些可能的状态：
...
在程序15.1中，大多数类方法都被定义为内联的。

Q：友元类是如何使用“基类”的方法的？

A：以书中例程为例，除构造函数外，所有的Remote类的方法都将一个Tv对象引用作为参数，这样友元类就可以使用“基类”的方法了。
...
小结：本小节主要定义了嵌套类的概念、语法和使用方法等。

Q：嵌套类的定义？

A：在C++中，可以将类声明放在另一个类中。在另一个类中声明的类被称为嵌套类（nested class），它通过提供类新的类型类作用域来避免名称混乱。包含类的成员函数可以创建和使用被嵌套类的对象；而仅当声明位于公有部分，才能在包含类的外面使用嵌套类，而且必须使用作用域解析运算符。

Q：嵌套类和包含这二者的概念有何区别？

A：对类进行嵌套并与包含并不同。包含意味着将类对象作为另一个类的成员，而对类进行嵌套不创建类成员，而是定义了一种类型，该类型仅在包含嵌套类声明的类中有效。
...
这两个Node类将在两个独立的QueueTP类中定义，因此不会发生名称冲突。即一个节点的类型为QueueTP::Node，而另一个节点的类型为QueueTP::Node。
...
A：一般假设用户的操作都是按照我们的要求进行的并且环境正常运行，但是这些并非总是如我们所愿，我们将用户的操作和环境的出现的问题叫做异常。

Q：异常处理的基本思想？

A：函数的调用方向和异常传播方向刚好相反。将不同的模块进行分工，有的模板专注于执行主要的逻辑，有的模板专注于容错处理。
...
catch块有点类似于函数定义，但并不是函数定义。关键字catch表明这是一个处理程序，而char∗∗s则表明该处理程序与字符串异常匹配。s与函数参数定义极其类似，因为匹配的引发将被赋给s。另外，当异常与该处理程序匹配时，程序执行括号中的代码。
...
dynamic_cast运算符已经在前面介绍过了。总之，假设High和Low是两个类，而ph和ph的类型分别为High∗∗和Low∗∗，而仅当Low是High的可访问积累（直接或间接）时，下面语句才将一个Low∗∗指针赋给pl:
...
正如您看到的，调用change()时，修改了pop1，但没有修改pop2。在change()中，指针被声明为const int ∗∗，因此不能用来修改指向的int。指针pc删除了const特征，因此可以用来修改指向的值，但仅当指向的值不是const才行。因此，pc可用于修改pop1，但不能用于修改pop2。
